...
function delete_category()
{
    ...
    if (isset($_GET["delete_cat"])) {
        ...
        if (!$result) {
            die("Could not delete specified data" . mysqli_error($connection));
        } else {
...
function add_post()
{
    ...
    if (isset($_POST["publish"])) {
        ...
        $query = "INSERT INTO
        posts(post_title,post_author,post_category,post_category_id,post_content,post_tags,post_status,post_date,post_image,post_views,post_comment_count)VALUES('$post_title','$post_author','$post_category','$post_category_id','$post_content','$post_tags','$post_status','$date','$target_file','$post_views','$post_comment_count')";

        $result = mysqli_query($connection, $query);

        if (!$result) {
            die("Error sending data" . "<br>" . mysqli_error($connection));
...
function show_posts()
{
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        echo "<td><img src='images/{$post_image}' width='65px'></td>";
